...
//		File file = new File("C:\\Users\\TheComputer\\Downloads\\test.txt.enc");
		File file = new File("temp/test.txt.enc");
		decrypt(file, "temp/rsa.pub");
	}
	...
	public static File decrypt(File inputFile, String key) throws Exception{
		...
		try (FileInputStream in = new FileInputStream(inputFile)) {
			...
			System.out.println(fileExists(decrypted));
			String fn = decrypted.getName();
